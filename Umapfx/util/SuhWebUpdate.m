...
classdef SuhWebUpdate < handle
    ...
    methods
        function this=SuhWebUpdate(app)
            [this.v, this.localVersion, this.appName, this.exeFile,...
                versionFile, remoteFolder]=app.getAppDetails;
            ...
            this.urlVersionFile=fullfile(remotePath, versionFile);
            this.urlHome='http://www.cytogenie.org';
        end
        
        function ok=isReachable(this)
            ok=edu.stanford.facs.util.ServicesUtil.isReachable(...
        ...
        function ask(this)
            questDlg(struct('msg', Html.WrapHr(...
                ...
                'modal', false, 'pauseSecs', 8), 'Get new update?', ...
    ...
    methods(Static)
        function Check(app, userIsAsking, waitSecs, helpNow, helpBefore)
            if nargin<1
                userIsAsking=false;
            end
            ...
            function check
                wu=SuhWebUpdate(app);
                [yesNewVersion, connectedOk]=wu.hasNew;
                if connectedOk
                    if yesNewVersion
                        pu.close;
                        wu.ask;
